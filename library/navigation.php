 ...
 */
if ( ! function_exists( 'semantique_top_bar_r' ) ) {
	function semantique_top_bar_r() {
		wp_nav_menu( array(
			'container'      => false,
			...
			'depth'          => 3,
			'fallback_cb'    => false,
			'walker'         => new semantique_Top_Bar_Walker(),
		));
	}
}
 ...
 */
if ( ! function_exists( 'semantique_mobile_nav' ) ) {
	function semantique_mobile_nav() {
		wp_nav_menu( array(
			'container'      => false,                         // Remove nav container
			'menu'           => __( 'mobile-nav', 'semantique' ),
			'menu_class'     => 'vertical menu',
			'theme_location' => 'mobile-nav',
			'items_wrap'     => '<ul id="%1$s" class="%2$s" data-accordion-menu>%3$s</ul>',
			'fallback_cb'    => false,
			'walker'         => new semantique_Mobile_Walker(),
		));
	}
}
...
*/
if ( ! function_exists( 'semantique_add_menuclass' ) ) {
	function semantique_add_menuclass( $ulclass ) {
		$find = array('/<a rel="button"/', '/<a title=".*?" rel="button"/');
		$replace = array('<a rel="button" class="button"', '<a rel="button" class="button"');

		return preg_replace( $find, $replace, $ulclass, 1 );
	}
	add_filter( 'wp_nav_menu','semantique_add_menuclass' );
}
...
if ( ! function_exists( 'semantique_breadcrumb' ) ) {
	function semantique_breadcrumb( $showhome = true, $separatorclass = false ) {
		...
		if ( ! is_front_page() ) {
			...
			if ( is_single() && ! is_attachment() ) {
				...
				// Paginated archives
				echo '<li class="current item-current-' . get_query_var('paged') . '">' . __('Page', 'semantique' ) . ' ' . get_query_var('paged') . '</li>';
